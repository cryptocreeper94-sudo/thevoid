...
- **Trust Layer Hallmark System**: Full ecosystem hallmark implementation with prefix `VO`. Genesis hallmark `VO-00000001` auto-created on server startup, referencing parent genesis `TH-00000001`. Tier 1 hallmarks (formal records) for subscriptions/purchases. Tier 2 trust stamps (lightweight audit trail) for auth, wallet, and subscription events with standardized categories. SHA-256 hashing, simulated blockchain tx/block. Public verification at `/api/hallmark/:id/verify` and genesis at `/api/hallmark/genesis`. Schema: `hallmarks`, `trust_stamps`, `hallmark_counter` tables. Service at `server/hallmark.ts`.
- **Affiliate Program**: Universal ecosystem affiliate system. Every user gets a `uniqueHash` for cross-platform referral links (`/ref/:hash`). 5 commission tiers (Base 10%, Silver 12.5%, Gold 15%, Platinum 17.5%, Diamond 20%). Affiliate dashboard at `/affiliate` with stats, tier progress, cross-platform links, payout requests (min 10 SIG). Endpoints: `/api/affiliate/dashboard`, `/api/affiliate/link`, `/api/affiliate/track` (public), `/api/affiliate/request-payout`. Schema: `affiliate_referrals`, `affiliate_commissions` tables. Service at `server/affiliate.ts`.
